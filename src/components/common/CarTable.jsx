...
import { Card, Flex, Table, Typography, Space, Button } from 'antd';
...
export const CarTable = ({
  ...
  const dataSource = [
    ...
    Object.assign({ key: 'auditHistory', prop: '📜 İlan Geçmişi' }, Object.fromEntries(cars.map(car => {
      if (!car.auditHistory) return [car.listingId, '—'];
      const historyStr = car.auditHistory.map(h => {
        const dateStr = h.auditDate ? new Date(h.auditDate).toLocaleDateString('tr-TR') : '?';
        return `[${dateStr}] ${h.action} — ${h.detail}`;
      }).join('\n');
      return [car.listingId, formatNotes(historyStr.split('\n'))];
    }))),
  ...
  const criticalFeatures = equipmentRules.filter(feature => feature.score > 0).sort((a,b)=>b.score-a.score||(b.price*b.score)-(a.price*a.score));
  const criticalSource = criticalFeatures.map(feature => ({
    key: `feat_${feature.name}`,
    prop: <Space size={4}>{feature.score > 0 && <StarRating count={feature.score}/>}<Text>{feature.name} <Text type="secondary" style={{ fontSize: '11px' }}>({feature.code})</Text></Text></Space>,
  ...
  const otherFeatures = equipmentRules.filter(feature => feature.score === 0).sort((a,b)=>b.price-a.price);
  const otherSource = otherFeatures.map(feature => ({
    key: `feat_${feature.name}`,
  ...
  return (
    <Flex vertical gap="large">
      ...
      <Card title={`⭐ Kritik Donanımlar ${yearLabel}`}>
        <Table dataSource={criticalSource} columns={columns} pagination={false} size="small" scroll={{ x: 'max-content' }} showHeader={false} rowHoverable={false} />
      </Card>

      <Card title={`Diğer Donanımlar ${yearLabel}`}>
        <Table dataSource={otherSource} columns={columns} pagination={false} size="small" scroll={{ x: 'max-content' }} showHeader={false} rowHoverable={false} />
      </Card>
